...
static inline int Quiescence(int alpha,int beta,S_BOARD *pos,S_SEARCHINFO *info){
    ...
    for(moveNum=0;moveNum<list->count;++moveNum){
        ...
        //DELTA PRUNING
        if ((stand_pat+pieceVal[CAPTURED(list->moves[moveNum].move)]+200<alpha) &&
            (PROMOTED(list->moves[moveNum].move)==0) &&
            (pos->material[pos->side] + pieceVal[CAPTURED(list->moves[moveNum].move)]>ENDGAME)){
                continue;
        }
...
static inline int AlphaBeta(int alpha,int beta,int depth,S_BOARD *pos,S_SEARCHINFO *info,int doNULL){
    ...
    if(!inCheck && !pvNode){
        ...
        //evaluation pruning
        if(depth < 3 && abs(beta-1)>-INFINITE+100){
        ...
        //futility pruning
        if(depth<4 && abs(alpha)< (INFINITE-1000) && staticEval+futilityMargin[depth] <= alpha){
            futilityPruning=1;
        }
    }
    //////////////////////////////////////
...
void SearchPosition(S_BOARD *pos,S_SEARCHINFO *info){
    ...
    if(bestMove == NOMOVE){
        for(currentDepth=1;currentDepth<=info->depth;++currentDepth){
            ...
            if ((bestScore==INFINITE-1) && bestMove != NOMOVE){
                break;
            }
        }
    }
